...
/// <summary>
/// Provides static methods to handle encryption and decryption of data from "musicroom.dat" (text data) files from Touhou 7.5. This class cannot be inherited.
/// </summary>
public static class MusicRoom
{
	...
	public static Span<byte> Encrypt(ReadOnlySpan<byte> data)
	{
		...
		Crypto.Crypt(outputData, 0x5C, 0x5A, 0x3D);
	...
	public static void Encrypt(Stream inputStream, Stream outputStream)
	{
		...
		Crypto.Crypt(data, 0x5C, 0x5A, 0x3D);
	...
	public static async ValueTask EncryptAsync(Stream inputStream, Stream outputStream)
	{
		...
		Crypto.Crypt(data.Span, 0x5C, 0x5A, 0x3D);
