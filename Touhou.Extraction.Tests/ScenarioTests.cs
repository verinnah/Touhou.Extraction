...
public sealed class ScenarioTests : IDisposable
{
	...
	private const string OUTPUT_PATH = $"{TEST_PATH}\\patchouli-test.sce";

	[Theory]
	[InlineData($"{TEST_PATH}\\patchouli.sce", 0xc9f64c7a4644c9f5)]
	public void Decrypt(string path, ulong hash)
	{
		...
		Scenario.Decrypt(fileStream, decryptedStream);
	...
	[Theory]
	[InlineData($"{TEST_PATH}\\patchouli.sce", 0xc9f64c7a4644c9f5, true)]
	public async Task DecryptAsync(string path, ulong hash, bool writeToDisk)
	{
		...
		await Scenario.DecryptAsync(fileStream, decryptedStream);
		...
		if (writeToDisk)
		{
			string entryPath = Path.Combine(TEST_PATH, "scenario.txt");
	...
	[Theory]
	[InlineData($"{TEST_PATH}\\scenario.txt", 0xabdfec88a8f8fb80, 0xc9f64c7a4644c9f5)]
	public void Encrypt(string path, ulong encryptedHash, ulong decryptedHash)
	{
		...
		using (FileStream inputStream = new(path, FileUtils.OpenReadFileStreamOptions))
		{
			...
			Scenario.Encrypt(inputStream, outputStream);
		}
	...
	public async Task EncryptAsync(string path, ulong encryptedHash, ulong decryptedHash)
	{
		...
		await using (FileStream inputStream = new(path, FileUtils.AsyncOpenReadFileStreamOptions))
		{
			...
			await Scenario.EncryptAsync(inputStream, outputStream);
		}
